...
impl QuickRegister {
    pub async fn new(endpoints: Vec<String>) -> Result<Self> {
        ...
        let client = BittensorClient::new(primary).await?;
        let extra_rpcs: Vec<String> = iter.collect();
        Ok(Self { client, extra_rpcs })
    }
    ...
    pub async fn register_to_subnet(
        ...
        if submit_on_new_head {
            ...
            self.client.wait_for_next_head(head_delay_ms).await?;
        }
